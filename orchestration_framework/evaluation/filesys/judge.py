...
async def async_judge(client: AsyncOpenAI, replay_dir: str, traj: str, tool_description: str) -> str:

    def get_prompt(fs_stat: str, query: str, traj: str, tool_description: str) -> str:
        input_a = fs_stat
    ...
    async def async_call_json(client: AsyncOpenAI, user_pt: str) -> str:
        sys_pt = Path(CUR_DIR.parent / "config" / "filesys" / "judge_sys_prompt.txt").read_text()
        resp = await client.chat.completions.create(
            model=os.environ.get("JUDGE_MODEL", "gpt-5"),
            reasoning_effort="low",
            prompt_cache_key="please-cache-please-filesys",
            messages=[
    ...
    fs_stat = await replay.replay(traj_json["messages"])
    query = traj_json["messages"][0]["content"]
    user_pt = get_prompt(fs_stat, query, traj, tool_description)
    result = await async_call_json(client, user_pt)
    result = dumps_scores_first(json.loads(result))
    return result


async def async_nosum_judge(client: AsyncOpenAI, replay_dir: str, traj: str, tool_description: str) -> str:
    try:
        traj_json = json.loads(traj)
        ...
        result = await async_judge(client, replay_dir, traj, tool_description)
        return result
...
async def main():
    ...
    replay_dir = Path(CUR_DIR.parent / "config" / "filesys" / "workspace").resolve()
    trajs = [line.strip() for line in open(args.trajs).readlines()]
    if args.end is None:
        trajs = trajs[args.start:]
    else:
        trajs = trajs[args.start:args.end]
    tool_description = json.load(open(args.tool_description, "r"))
    ...
    with open(args.output, "w") as f:
        for i in range(0, len(trajs), max_concurrent):
            ...
            for traj in batch:
                coroutine_list.append(async_nosum_judge(client, replay_dir, traj, tool_description))
