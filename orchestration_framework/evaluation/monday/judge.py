...
async def async_judge(client: AsyncOpenAI, gt: str, traj: str, tool_description: str) -> str:
    ...
    async def async_call_json(client: AsyncOpenAI, user_pt: str) -> str:
        sys_pt = Path(CUR_DIR.parent / "config" / "monday" / "judge_sys_prompt.txt").read_text()
...
async def main():
    ...
    tool_description = json.load(Path(CUR_DIR.parent / "config" / "monday" / "tool_list.json").open())
    gt = Path(CUR_DIR.parent / "config" / "monday" / "ground_truth.txt").read_text()
